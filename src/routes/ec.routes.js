...
function validateEvaluationScales(evaluations) {
  ...
  for (const evaluation of evaluations) {
    const echelle = Number(evaluation.echelle);
    if (!['CC', 'TP', 'SN'].includes(evaluation.type) || types.has(evaluation.type) || !Number.isFinite(echelle) || echelle <= 0 || echelle > 100) {
      throw Object.assign(new Error('Les barèmes CC, TP et SN doivent être compris entre 0 et 100.'), { status: 400 });
    }
    types.add(evaluation.type); total += echelle;
  }
  if (Math.abs(total - 100) > 0.001) throw Object.assign(new Error(`La somme des barèmes doit être égale à 100 (actuellement ${total}).`), { status: 400 });
}
...
router.post('/ue/:idue', requirePermission('ue_ec.manage'), async (req,res,next)=>{
  try{const {intitule,credit,evaluations=[]}=req.body;if(!credit)return res.status(400).json({message:'credit requis.'});validateEvaluationScales(evaluations);const c=await pool.getConnection();try{await c.beginTransaction();const [r]=await c.query('INSERT INTO ec (IDUE,INTITULE,CREDIT,created_at,updated_at) VALUES (?,?,?,?,NOW())',[req.params.idue,intitule||null,credit,new Date()]);for(const ev of evaluations)await c.query('INSERT INTO ec_evaluation_types (IDEC,type,echelle) VALUES (?,?,?)',[r.insertId,ev.type,ev.echelle]);await c.commit();res.status(201).json({id:r.insertId})}catch(e){await c.rollback();throw e}finally{c.release()}}catch(e){next(e)}
});
router.get('/ue/:idue', async(req,res,next)=>{try{const ecs=await query('SELECT e.*,JSON_ARRAYAGG(JSON_OBJECT(\'type\',t.type,\'echelle\',t.echelle)) evaluations FROM ec e LEFT JOIN ec_evaluation_types t ON t.IDEC=e.IDEC WHERE e.IDUE=? GROUP BY e.IDEC',[req.params.idue]);res.json(ecs)}catch(e){next(e)}});
router.put('/:idec/notes', requirePermission('ec_notes.write'), async (req,res,next)=>{
  const c=await pool.getConnection(); try { const {matricule,classe_id,annee,note_cc,note_tp,note_sn,motif}=req.body; if(!matricule||!classe_id||!annee||!motif)return res.status(400).json({message:'matricule, classe_id, annee et motif requis.'});
    ...
    const [types]=await c.query('SELECT type,echelle FROM ec_evaluation_types WHERE IDEC=?',[ec.IDEC]);let total=0;for(const type of types){const value=type.type==='CC'?note_cc:type.type==='TP'?note_tp:note_sn;if(value===undefined||value===null) return res.status(400).json({message:`Note ${type.type} requise.`});if(Number(value)<0||Number(value)>Number(type.echelle))return res.status(400).json({message:`La note ${type.type} doit être comprise entre 0 et ${type.echelle}.`});total+=Number(value)} const moyenne=total;
