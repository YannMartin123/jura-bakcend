...
(async () => { const connection = await pool.getConnection(); try {
  ...
  const [result] = await connection.query("INSERT INTO users (username,name,email,password,password_changed_at,locale,is_active,created_at,updated_at) VALUES (?,?,?,?,NULL,'fr',1,NOW(),NOW())", [username,name,email,hash]);
